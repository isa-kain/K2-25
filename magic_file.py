...
def ind_analysis(list_of_files, dataset):
    ...
    for thisfile in list_of_files:
        ...
        if 'M' in dataset:
            x, y, yerr = np.genfromtxt(thisfile, delimiter=',', dtype=None, unpack=True, usecols=(0,1,2))

            t0 = 2457062.57964
            per = 3.484564
            numtransits = np.abs(round((x[0]-t0)/per))
            ...
            data = ascii.read(thisfile, delimiter = '\s', header_start = None, data_start = 0, guess = False)
            x = np.array(data['col1']) + 2454833.    ##['BMJD_OBS'] + 2400000. + .5
            y = data['col2'] ##normalized flux
            ...
            data = ascii.read(thisfile, delimiter = '\s', header_start = 0, data_start = 1, guess = False)
            x = np.array(data['Time'])
            y = np.array(data['Fl'])
            yerr = np.array(data['Fl_err'])

            test = thisfile.split('_')[2]
            if len(test.split('-'))>1:
            ...
            data = ascii.read(thisfile, delimiter = '\s', guess = False, format = 'cds')
            ...
            reduction = thisfile.split('_')[2]
            fc.reduction = reduction
            x, y, yerr = np.genfromtxt(thisfile, delimiter=' ', dtype=None, skip_header=1, unpack=True, usecols=(0,1,2))
            conv_flux = False
...
def megastack_analysis(list_of_files, dataset):
    ...
    if 'M' in dataset:
        ...
        t0 = 2457062.57964
        per = 3.484564
        numtransit = np.abs(round((t[0]-t0)/per))
        ...
        ## Determine time of mid transit for first datafile
        data = ascii.read(list_of_files[0], delimiter = '\s', data_start = 0, guess = False)
...
def visual_analysis(dataset):
    ...
    if 'M' in dataset:
        ...
        f=open('K2_slope_file.txt',"r")
    elif 'L' in dataset:
        f=open('LCO_slope_file.txt',"r")
